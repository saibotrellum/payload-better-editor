...
- Selecting a block no longer scrolls the preview on every keystroke - only an actual block mutation scrolls.

### Changed
- `LivePreviewDataChannel` renders nothing and sends nothing. Its whole job is to occupy `views.edit.livePreview`, which is what stops Payload from mounting a second preview iframe against the ref the overlay owns; the unsaved draft goes out through `useIsolatedDraft`, because what it posts is not the form state - block order, additions and deletions live in the isolated draft, and posting form state over the top put the old block list back. The `livePreviewData` option keeps its name and its default, and consumers see no behavioural change.
- Block rows carry a named `DraftBlock` type rather than `Record<string, any>`. Values are `unknown`: the shape is whatever the host's block config declares, so the plugin cannot name it, but every read has to narrow. Exported, so a consumer typing its own handlers can use it.
- Lint runs clean again. It had 13 errors, which is not cosmetic here - the release workflow gates `npm publish` behind `pnpm lint`, so `integration` could not have published while they stood.
