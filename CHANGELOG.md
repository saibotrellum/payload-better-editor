...
### Fixed
- Prevented Payload CMS form-state rollbacks on block duplication: an internal `BlockRollbackGuard` component monitors `formState` row drops and automatically repairs unprompted state regressions dispatched by Payload during background re-validations or server roundtrips. Intentional block removals (`REMOVE_ROW`) are preserved.
- Stabilized preview frame bindings and URLs (`useStablePreviewURL`), avoiding unnecessary iframe re-mounts and toolbar disconnects when parent admin components re-render.
